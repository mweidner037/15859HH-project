...
// Balancing only: adds the second tree layer (an AVL tree for converting
// between nodes and indices) but not the third (SALMs).
// So it has O(n) insertions in the worst case
// because it can take that long to find a neighbor node when processing
// concurrent inserts at the same location.
...
export class CTextLognBalanced
